...
+ **讨论: 为什么在POMDP问题中的策略使用条件规划树的形式?**
  + 这是因为两点, 一是在POMDP中, 我们无法得知准确的状态信息, 而只能根据观察来进行决策, 可是观察是关于状态的一种残缺信息, 并不是我们直接作出决策的准确依据；而是在POMDP中, 决策往往需要借助历史信息和历史观察序列, 此时使用一个仅和当前观察或者信念状态有关的决策函数$\pi$就无法利用历史决策信息了
### 值迭代
+ 在POMDP中进行值迭代的想法
